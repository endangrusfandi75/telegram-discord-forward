...
async function pollTarget() {
  try {
    ...
    if (!primed) {
      ...
      lastSeenId = Math.max(lastSeenId, sorted[sorted.length - 1].id);
      primed = true;
      return;
    }
    ...
    console.error('Poll error for target channel:', e.message);
  }
}
