...
async function flushAlbum(groupedId) {
  ...
  for (const msg of messages) {
    ...
    try {
      ...
      const fileName = msg.message?.file?.name || `${msg.message?.id}.${(msg.message?.file?.mimeType?.split('/')[1]) || 'bin'}`;
...
async function startTelegram() {
  ...
  client.addEventHandler(async (event) => {
    ...
    if (chatId !== TELEGRAM_TARGET_CHAT_ID) {
      console.error(`[debug] Received message from chatId=${chatId} (target=${TELEGRAM_TARGET_CHAT_ID}), skipped`);
      return; // filter by target chat
    }
    ...
    // Text only: msg.message is the raw text string.
